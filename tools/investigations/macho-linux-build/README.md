...
- `macho_section_probe_compat.h` is used only by the `start_kernel()` dependency probe. It preserves generic `__section()` Linux section names under a dedicated `__ORLIX` Mach-O segment so the probe can separate generic section syntax from direct section attributes, percpu section construction, and Mach-O's 16-byte section-name limit. It is not the final product section/linker policy.
