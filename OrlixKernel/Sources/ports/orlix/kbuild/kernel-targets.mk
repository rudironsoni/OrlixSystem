...
__kernel-archive: __prepare-kbuild
	...
	{ for src_rel in $(ORLIX_KERNEL_LINUX_SOURCES); do case "$$src_rel" in drivers/base/devres.c|fs/seq_file.c|kernel/cpu.c|kernel/locking/mutex.c|kernel/panic.c|kernel/rcu/update.c|kernel/sched/completion.c|kernel/sched/swait.c|kernel/softirq.c|kernel/time/time.c|mm/maccess.c|mm/mmzone.c|mm/page_alloc.c|mm/swap.c|mm/util.c) printf '%s\n' "$(ORLIX_PRODUCT_ADAPTER_SOURCE)/$$src_rel" ;; *) printf '%s\n' "$(ORLIX_KERNEL_PORT_DIR)/$$src_rel" ;; esac; done; } > "$(ORLIX_KERNEL_ARCHIVE_MANIFEST)"; \
	...
	compile_slice() { \
		...
		contract_dir="$$output_dir/archive-contract"; \
		archive="$$output_dir/$(ORLIX_KERNEL_ARCHIVE_NAME)"; \
		rm -rf "$$output_dir"; \
		mkdir -p "$$obj_dir"; \
		objs=(); \
		for src_rel in $(ORLIX_KERNEL_LINUX_SOURCES); do \
			case "$$src_rel" in drivers/base/devres.c|fs/seq_file.c|kernel/cpu.c|kernel/locking/mutex.c|kernel/panic.c|kernel/rcu/update.c|kernel/sched/completion.c|kernel/sched/swait.c|kernel/softirq.c|kernel/time/time.c|mm/maccess.c|mm/mmzone.c|mm/page_alloc.c|mm/swap.c|mm/util.c) src="$(ORLIX_PRODUCT_ADAPTER_SOURCE)/$$src_rel" ;; *) src="$(ORLIX_KERNEL_PORT_DIR)/$$src_rel" ;; esac; \
			...
			orlix_product_adapter_verify_object_contract "$$obj" "$$contract_dir"; \
			objs+=("$$obj"); \
		done; \
		orlix_product_adapter_generate_boundaries "$$contract_dir" "$$platform" "$$target"; \
