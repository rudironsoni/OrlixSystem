/* OrlixKernel/arch/darwin/signal_bridge.c
 ...
 * between Darwin types and Orlix internal types.
 *
...
#ifndef ORLIX_SIG_NUM
#define ORLIX_SIG_NUM 64
#endif
#ifndef ORLIX_SIG_NUM_WORDS
#define ORLIX_SIG_NUM_WORDS ((ORLIX_SIG_NUM + 63) / 64)
#endif
...
void bridge_sigset_from_host(const sigset_t *host_set, struct signal_mask_bits *out_set) {
    ...
    for (int sig = 1; sig <= 64; sig++) {
        if (sigismember(host_set, sig)) {
            ...
            int bit = (sig - 1) % 64;
            if (idx < ORLIX_SIG_NUM_WORDS && bit < 64) {
...
void bridge_sigset_to_host(const struct signal_mask_bits *internal_set, sigset_t *host_set) {
    ...
    for (int sig = 1; sig <= 64; sig++) {
        int idx = (sig - 1) / 64;  // Fixed: signal 64 now maps to idx 0, bit 63
        int bit = (sig - 1) % 64;
        if (idx < ORLIX_SIG_NUM_WORDS && (internal_set->sig[idx] & (1ULL << bit))) {
