...
**Architecture:** Add an in-repo `clang-tidy` plugin that scans translation units with path-aware IXLand policy checks. `scripts/lint_linux_surface.sh` becomes a pure `clang-tidy` runner that builds the plugin, generates `compile_commands.json`, and runs only the IXLand checks. `scripts/lint_linux_vendor_headers.sh` is replaced by a repo-layout validator for `third_party/linux` and `project.yml` topology, not source lint.
...
**Files:**
- Create: `tools/clang_tidy_ixland/CMakeLists.txt`
- Create: `tools/clang_tidy_ixland/IXLandTidyModule.cpp`
- Create: `tools/clang_tidy_ixland/IXLandSourcePolicyCheck.h`
- Create: `tools/clang_tidy_ixland/IXLandSourcePolicyCheck.cpp`
- Create: `tools/clang_tidy_ixland/IXLandTestPolicyCheck.h`
- Create: `tools/clang_tidy_ixland/IXLandTestPolicyCheck.cpp`
- Create: `.clang-tidy`

- [ ] Define the plugin build shape against Homebrew LLVM/Clang CMake exports.
- [ ] Register an `ixland-source-policy` check and an `ixland-test-policy` check inside one custom module.
- [ ] Set `.clang-tidy` to run only the IXLand checks by default.

### Task 2: Move Linux-owner source policy into clang-tidy

**Files:**
- Modify: `tools/clang_tidy_ixland/IXLandSourcePolicyCheck.h`
- Modify: `tools/clang_tidy_ixland/IXLandSourcePolicyCheck.cpp`
- Reference: `scripts/lint_linux_surface.sh`
- Reference: `AGENTS.md`

- [ ] Implement path classification for Linux-owner files under `IXLandKernel/fs`, `IXLandKernel/kernel`, `IXLandKernel/runtime`, and `IXLandKernel/include`.
- [ ] Implement forbidden-include checks for host frameworks, `IXLandHostAdapter/**`, `IXLandMLibC/**`, and banned host headers.
...
**Files:**
- Modify: `tools/clang_tidy_ixland/IXLandTestPolicyCheck.h`
- Modify: `tools/clang_tidy_ixland/IXLandTestPolicyCheck.cpp`
- Reference: `scripts/lint_linux_surface.sh`

- [ ] Implement path classification for `IXLandKernelTests` and `IXLandHostAdapterTests`.
...
- Create: `scripts/build_ixland_clang_tidy_module.sh`

- [ ] Make `scripts/lint_linux_surface.sh` do only:
  - build the plugin
  - generate `compile_commands.json`
  - run `clang-tidy` with `-load` and the IXLand-only checks
...
- Modify: `docs/plans/01-IXLandKernel_IXLandHostAdapter_Split_Plan.md`
...
- [ ] Verify with:
  ...
  - `xcodebuild test -project IXLandKernel.xcodeproj -scheme IXLandKernel-6.12-arm64 -configuration Debug -destination 'platform=iOS Simulator,id=63FEBB50-E358-47C6-A8C2-C77E2A391BB2' -derivedDataPath /private/tmp/IXLandDerivedData -only-testing:IXLandKernelTests`
  - `xcodebuild test -project IXLandKernel.xcodeproj -scheme IXLandKernel-6.12-arm64 -configuration Debug -destination 'platform=iOS Simulator,id=63FEBB50-E358-47C6-A8C2-C77E2A391BB2' -derivedDataPath /private/tmp/IXLandDerivedData -only-testing:IXLandHostAdapterTests`
