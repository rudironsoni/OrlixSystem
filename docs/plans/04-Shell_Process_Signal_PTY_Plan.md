...
- public `getpgrp()` / `getpgid()` / `setpgid()` / `getsid()` / `setsid()` now have simulator-proven Linux-surface coverage for process-group and session wrapper behavior, not just the internal owner path
