...
The plan must stage proof-driven work for named environments, import-to-enter flow, OCI image layout import, Linux substrate proof, host-folder mounts, virtio-fs, Linux oracle expansion, networking, namespaces, virtual cgroups, native performance, OCI Runtime config/schema/lifecycle behavior, truthful OCI feature reporting, `orlix run`, and registry pull tooling. OCI image import may advance as OrlixOS data-input work, but OCI Runtime lifecycle, feature, and compatibility claims must wait for the Linux-owned substrate they depend on. Each phase must name repo changes, likely files or areas, tests, proof criteria, risks, blockers, and explicit non-actions.
