...
__macho-start-kernel-dependency-probe: __prepare-kbuild
	...
	set +e; \
	/usr/bin/env -u SDKROOT "$$cc" -target "$(ORLIX_MACHO_IOS_SIMULATOR_TARGET)" -isysroot / -x c -ffreestanding -fno-builtin -fno-stack-protector -fno-objc-arc -fno-common -nostdinc -D__KERNEL__ -include "$(ORLIX_KERNEL_PORT_DIR)/include/linux/compiler-version.h" -include "$(ORLIX_KERNEL_PORT_DIR)/include/linux/kconfig.h" -I"$(ORLIX_KERNEL_PORT_DIR)/arch/$(LINUX_ARCH)/include" -I"$(ORLIX_KERNEL_BUILD_DIR)/arch/$(LINUX_ARCH)/include/generated" -I"$(ORLIX_KERNEL_PORT_DIR)/include" -I"$(ORLIX_KERNEL_BUILD_DIR)/include" -I"$(ORLIX_KERNEL_PORT_DIR)/arch/$(LINUX_ARCH)/include/uapi" -I"$(ORLIX_KERNEL_BUILD_DIR)/arch/$(LINUX_ARCH)/include/generated/uapi" -I"$(ORLIX_KERNEL_PORT_DIR)/include/uapi" -I"$(ORLIX_KERNEL_BUILD_DIR)/include/generated/uapi" -MMD -MF "$$probe_dir/init_main.d" -c "$(ORLIX_KERNEL_PORT_DIR)/init/main.c" -o "$$probe_dir/init_main.o" > "$$log" 2>&1; \
	rc="$$?"; \
	set -e; \
	if [ "$$rc" -eq 0 ]; then \
		printf '%s\n' 'compiled' > "$$status"; \
